...
public class OrderRecordResource extends
	...
	private Logger logger = LoggerFactory
			.getLogger("com.dta.resource.OrderRecordResource");
	...
	public Response getCoPreRecord(@BeanParam CoachPrecontractRecordVo vo) {
		try {
			...
			int rows = vo.getRows();
			/*
			 * if (rows != 0) vo.setPage((page + 1) * rows - rows);
			 */
			List<CoachPrecontractRecord> resultList = service
					.getCoPreRecord(vo);
	...
	public Response getLatestRrecontractRecordSize(
			@BeanParam CoachPrecontractRecordVo vo) {
		try {
			return Response.status(200)
					.entity(service.getLatestRrecontractRecordSize(vo)).build();
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Produces(MediaType.TEXT_PLAIN)
	public Response getLastestCancelRecordSize(
			@BeanParam CoachPrecontractRecordVo vo) {
		try {
			System.out.println("********** in orderRecordResource"
					+ vo.getLastQueryTime());
			return Response.status(200)
					.entity(service.getLastestCancelRecordSize(vo)).build();
		} catch (Exception e) {
			e.printStackTrace();
			return Response.status(500).entity(-1).build();
		}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response getEvaluationInfoByOrderId(
			@QueryParam("order_id") Integer order_id) {
		try {
			return Response.status(200)
					.entity(service.getEvaluationInfoByOrderId(order_id))
					.build();
		} catch (Exception e) {
			e.printStackTrace();
			return Response
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response getCoachFinanceSumInfo(
		...
		try {
			...
			List<CoachFianceSummarizing> list = service
					.getCoachFinanceSumInfo(coach_id);
			for (int i = 0; i < list.size(); ++i) {
				...
				if (list.get(i).getStudent_level() != null
						&& list.get(i).getStudent_level() != 0)
					++VIPStudentNumbers;
			}
			...
			if (rightTime) {
				...
				if (service.updateObjectById(orderRecord) == 1)
					return Response
							...
							.entity(new ResultBean(
									GlobalConstant.OPERATION_SUCCESS,
									GlobalConstant.COACHSCANNEDSUCCESS))
							.build();
			}
			return Response
					.status(200)
					.entity(new ResultBean(GlobalConstant.OPERATION_FAIL,
							GlobalConstant.COACHSCANNEDFAIL)).build();
		...
		try {
			...
			if (service.updateObjectById(orderRecord) != 1)
				return Response
						.status(200)
						.entity(new ResultBean(GlobalConstant.OPERATION_FAIL,
								GlobalConstant.CACELPRECONTRACTFAIL)).build();
			return Response
					.status(200)
					.entity(new ResultBean(GlobalConstant.OPERATION_SUCCESS,
							GlobalConstant.CACELPRECONTRACTSUCCESS)).build();
		} catch (Exception e) {
			logger.error(e.getMessage());
			e.printStackTrace();
			return Response
