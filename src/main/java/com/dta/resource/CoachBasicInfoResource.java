...
public class CoachBasicInfoResource extends BaseAllResource<CoachBasicInfo, CoachBasicInfo>{
	private ICoachBasicInfoService service = (ICoachBasicInfoService)ServiceProvider.getBean("coachBasicInfoServiceImpl");
	...
	public Response addWithPhoto() {
		...
		if (!isMultipart) {
			return Response.status(200).entity(new ResultBean(GlobalConstant.OPERATION_FAIL, GlobalConstant.NOFILE))
					.build();
		} else {
			try {
				...
				while (iterator.hasNext()) {
					...
					if (item.isFormField()) {
						MethodAccess access = MethodAccess.get(CoachBasicInfo.class);
						access.invoke(po, generateMethodName("set", name), Streams.asString(stream));
					} else {
