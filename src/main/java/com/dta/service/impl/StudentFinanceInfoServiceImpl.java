...
@Service
public class StudentFinanceInfoServiceImpl implements IStudentFinanceInfoService {
	@Autowired
	...
	@Override
	@Transactional
	public int updateBalanceByStudentId(Integer student_id, Double amount, Integer sign){
		StudentFinanceInfo studentFinaceInfo = dao.getStudentFinanceInfoById(student_id);
		if(studentFinaceInfo != null){
			Double oldAmount = studentFinaceInfo.getStudent_account_balance();
			studentFinaceInfo.setStudent_account_balance(oldAmount + amount * sign);
	...
	public int addOrUpdateStudentFinanceInfo(Integer student_id, Double amount){
		StudentFinanceInfo studentFinanceInfo = dao.getStudentFinanceInfoById(student_id);
